        ...
        try:
            async with httpx.AsyncClient(timeout=30) as client:
                ...
                if obj_stats.status_code != 200:
                    log.error('从diving-fish获取maimaiDX单曲数据失败。已切换至本地暂存文件')
                    chart_stats = await openfile(chart_file)
                else:
                    chart_stats = obj_stats.json()
                    await writefile(chart_file, chart_stats)
        except asyncio.exceptions.TimeoutError:
            log.error('从diving-fish获取maimaiDX单曲数据超时，正在使用yuzuapi中转获取单曲数据')
            chart_stats = await get_music_alias('chart')
            if isinstance(chart_stats, str):
                log.error('从yuzuapi获取maimaiDX单曲数据失败。已切换至本地暂存文件')
                chart_stats = await openfile(chart_file)
            else:
                await writefile(chart_file, chart_stats)
